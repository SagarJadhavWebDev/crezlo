            ...
            <div key={index}>
              {menu?.name && (
                <h3 className="text-xs uppercase text-gray-400 dark:text-gray-500 font-semibold pl-3">
                  ...
                  <span className="lg:hidden lg:sidebar-expanded:block 2xl:block">{menu?.name}</span>
                </h3>
              )}
              <ul className="mt-3">
                {menu.items?.map((item) => {
          ...
          {sidebarExpanded ? (
            <SidebarLink href={marketPlaceMenu?.href} active={segments?.includes(marketPlaceMenu?.value)}>
              <div className="flex items-center">
                ...
                <span
                  className={`text-md font-medium ml-4 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200 ${
