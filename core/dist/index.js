...
const AuthProvider = ({ children, config }) => {
    ...
    // Storage utilities
    const setStorageItem = require$$0.useCallback((key, value) => {
        try {
    ...
    const getStorageItem = require$$0.useCallback((key) => {
        try {
            if (mergedConfig.tokenStorage.storageType === "cookie" && storageManager) {
    ...
    const removeStorageItem = require$$0.useCallback((key) => {
        try {
            if (mergedConfig.tokenStorage.storageType === "cookie" && storageManager) {
    ...
    // Token management
    const saveTokens = require$$0.useCallback((tokens) => {
    ...
    const loadTokens = require$$0.useCallback(() => {
    ...
    const clearTokens = require$$0.useCallback(() => {
    ...
    // User management
    const saveUser = require$$0.useCallback((user) => {
    ...
    const loadUser = require$$0.useCallback(() => {
    ...
    const clearUser = require$$0.useCallback(() => {
    ...
    const register = require$$0.useCallback(async (data) => {
        try {
            dispatch({ type: "SET_LOADING", payload: true });
            dispatch({ type: "SET_ERROR", payload: null });
    ...
    const refreshToken = require$$0.useCallback(async () => {
        try {
            const tokens = loadTokens();
            if (!tokens?.refreshToken) {
    ...
    const updateUser = require$$0.useCallback(async (userData) => {
        try {
            dispatch({ type: "SET_LOADING", payload: true });
    ...
    const changePassword = require$$0.useCallback(async (oldPassword, newPassword) => {
        try {
            dispatch({ type: "SET_LOADING", payload: true });
    ...
    const hasRole = require$$0.useCallback((roleName) => {
        return state.user?.roles.some((role) => role.name === roleName) || false;
    }, [state.user]);
    const hasAnyRole = require$$0.useCallback((roleNames) => {
        return roleNames.some((roleName) => hasRole(roleName));
    }, [hasRole]);
    const hasAllRoles = require$$0.useCallback((roleNames) => {
        return roleNames.every((roleName) => hasRole(roleName));
    }, [hasRole]);
    const hasPermission = require$$0.useCallback((resource, action) => {
        return state.user?.permissions.some((permission) => permission.resource === resource && permission.action === action) || false;
    }, [state.user]);
    const hasAnyPermission = require$$0.useCallback((permissions) => {
        return permissions.some(({ resource, action }) => hasPermission(resource, action));
    }, [hasPermission]);
    const hasAllPermissions = require$$0.useCallback((permissions) => {
    ...
    const checkAuthStatus = require$$0.useCallback(async () => {
        try {
            dispatch({ type: "SET_LOADING", payload: true });
            const tokens = loadTokens();
            const user = loadUser();
