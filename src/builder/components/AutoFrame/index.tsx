...
const CopyHostStyles = ({
  ...
  useEffect(() => {
    ...
    // MUDASSIR CHANGES MADE TO HANDLE DYNAMIC HEIGHT AS COPYING FROM PARENT HTML
    if (doc.documentElement.style.overflow) {
...
function AutoFrame({ children, className, debug, id, onReady = () => {}, onNotReady = () => {}, frameRef, ...props }: AutoFrameProps) {
  ...
  return (
    <iframe
      ...
      className={className}
      id={id}
      srcDoc='<!DOCTYPE html><html><head></head><body><div id="frame-root" data-crezlo-entry></div></body></html>'
      ref={frameRef}
      onLoad={() => {
        setLoaded(true);
      }}
    >
      <autoFrameContext.Provider value={ctx}>
        {loaded && mountTarget && (
          <CopyHostStyles debug={debug} onStylesLoaded={() => setStylesLoaded(true)}>
